...
exports.book_delete_get = function(req, res, next) {
  ...
  async.parallel(
    {
      book: callback => Book.findById(id, callback),
      instances: callback => BookInstance.find({ book: id }, callback)
    },
    (err, results) => {
...
exports.book_delete_post = function(req, res, next) {
  ...
  Promise.all([
    ...
    .then(results => {
      const [book, instances] = results;
      if (!book) {
        const error = new Error("book doesnt exist");
        error.status = 404;
        return next(error);
      }
